...
public class Paragraph extends Job implements Cloneable, JsonSerializable {
  ...
  public List<InterpreterCompletion> completion(String buffer, int cursor) {
    ...
    try {
      ...
      logger.debug("CLDR: Unable to get completion because there's no interpreter bind to it", e);
      return null;
    }
    setText(buffer);
    ...
    try {
      ...
      logger.debug("CLDR: Fail to get completion", e);
  ...
  public boolean execute(boolean blocking) {
    ...
    try {
      ...
      logger.debug("CLDR : Error while executing the paragraph");
      throw new RuntimeException(e);
    }
  }
